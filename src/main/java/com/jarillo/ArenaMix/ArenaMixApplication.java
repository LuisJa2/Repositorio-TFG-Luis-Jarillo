...
public class ArenaMixApplication {
	...
	public CommandLineRunner probarBaseDeDatosCompleta(
		...
		return args -> {
			...
			// 1. Recuperar al Organizador o crearlo; si ya existe con contraseña plana, la migra a BCrypt
			Usuario organizador = usuarioRepo.findAll().stream()
					.filter(u -> u.getUsername().equals("AdminTest"))
					.findFirst()
					.map(u -> {
						if (!u.getPasswordHash().startsWith("$2")) {
							u.setPasswordHash(encoder.encode("secreta123"));
							return usuarioRepo.save(u);
						}
						return u;
					})
					.orElseGet(() -> {
						Usuario nuevo = new Usuario("AdminTest", "admin@test.com",
								encoder.encode("secreta123"), "ORGANIZADOR");
						return usuarioRepo.save(nuevo);
					});

			// 2. Igual para el jugador de prueba
			Usuario jugador = usuarioRepo.findAll().stream()
					...
					.orElseGet(() -> {
						Usuario nuevo = new Usuario("JugadorPro", "jugador@test.com",
								encoder.encode("1234"), "JUGADOR");
						return usuarioRepo.save(nuevo);
					});
